...
namespace aetherion {
    ...
    constexpr vk::QueueFlagBits toVkQueueFlag(const RenderQueueType type) {
        switch (type) {
            case RenderQueueType::Graphics:
                return vk::QueueFlagBits::eGraphics;
            case RenderQueueType::Compute:
                return vk::QueueFlagBits::eCompute;
            case RenderQueueType::Transfer:
                return vk::QueueFlagBits::eTransfer;
            case RenderQueueType::Present:
                // NOTE: Vulkan does not have a specific Present flag; presentation support is
                // queried separately.
                return {};
            default:
                throw std::invalid_argument("Invalid RenderQueueType");
        }
    }

    constexpr RenderQueueType toQueueType(const vk::QueueFlagBits type) {
        switch (type) {
            case vk::QueueFlagBits::eGraphics:
                return RenderQueueType::Graphics;
            case vk::QueueFlagBits::eCompute:
                return RenderQueueType::Compute;
            case vk::QueueFlagBits::eTransfer:
                return RenderQueueType::Transfer;
            default:
                throw std::invalid_argument("Invalid vk::QueueFlagBits");
        }
    }

    constexpr vk::QueueFlags toVkQueueFlags(const RenderQueueTypeFlags flags) {
        vk::QueueFlags vkFlags = {};

        if (flags.contains(RenderQueueType::Graphics)) {
            vkFlags |= vk::QueueFlagBits::eGraphics;
        }
        if (flags.contains(RenderQueueType::Compute)) {
            vkFlags |= vk::QueueFlagBits::eCompute;
        }
        if (flags.contains(RenderQueueType::Transfer)) {
            vkFlags |= vk::QueueFlagBits::eTransfer;
        }
        if (flags.contains(RenderQueueType::Present)) {
    ...
    constexpr RenderQueueTypeFlags toQueueTypeFlags(const vk::QueueFlags flags) {
        RenderQueueTypeFlags queueFlags = {};

        if (flags & vk::QueueFlagBits::eGraphics) {
            queueFlags |= RenderQueueType::Graphics;
        }
        if (flags & vk::QueueFlagBits::eCompute) {
            queueFlags |= RenderQueueType::Compute;
        }
        if (flags & vk::QueueFlagBits::eTransfer) {
            queueFlags |= RenderQueueType::Transfer;
        }
    ...
    constexpr vk::BufferUsageFlags toVkBufferUsageFlags(const RenderBufferType type) {
        switch (type) {
            ...
            default:
                throw std::invalid_argument("Invalid RenderBufferType");
        }
    }
